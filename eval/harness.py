...
Usage:
    ...
    python eval/harness.py --data eval/labeled_set.jsonl --mode ablation \
        --rubric-variants eval/rubric_v1.txt eval/rubric_v2.txt eval/rubric_v3.txt
...
from app.pipelines.importance import classify_importance, SYSTEM_PROMPT
...
def eval_importance(records: list[dict], rubric_override: str | None = None) -> dict:
    ...
    for r in labeled:
        ...
        if not text:
            continue
        # Use pain points from the record if available; otherwise run pipeline
        pain_points = [{"label": p, "similarity_score": 1.0} for p in r.get("pain_points", [])]

        if rubric_override:
            # Temporarily swap system prompt for ablation
            ...
            try:
                result = classify_importance(text, pain_points)
            finally:
...
def run_ablation(records: list[dict], rubric_paths: list[Path]) -> dict:
    """
    Run importance eval against multiple rubric variants.
    Returns comparison table: variant → agreement metrics.
    """
    results = {}

    # Baseline (current default rubric)
    logger.info("Evaluating baseline rubric...")
    results["baseline"] = eval_importance(records)
    ...
    for path in rubric_paths:
        ...
        results[variant_name] = eval_importance(records, rubric_override=rubric_text)
...
def main(
    ...
    if "ablation" in report:
        ab = report["ablation"]
        typer.echo(f"  best rubric variant: {ab['best_variant']}")
